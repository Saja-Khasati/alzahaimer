...
export const signup = async (req,res,next)=>{
    ...
    const c = await DoctorModel.findOne({
      where:{
          email:email,
      }
  });
  ...
  const hashedPassword =  bcrypt.hashSync(doctorpassword,parseInt(process.env.SALTROUND));
  const token= jwt.sign({email},process.env.CONFIRMEMAILSECRET,{expiresIn:'1h'});
  const html = `<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
   ...
   await sendEmail(email,"confirm Email",html)
     ...
     const createUser = await DoctorModel.create({name,email,password:hashedPassword})
    if(createUser.confirmEmail!=true){
     // const Link = `${req.protocol}://${req.headers.host}/doctor/delete`;
   return res.status(404).json({message:"not success plz confirm your email"})
    
    }
    return res.redirect(process.env.SIGNUP);          
  }
...
export const ConfirmEmail = async(req,res,next)=>{
    ...
    const d = await DoctorModel.update(
      ...
      if (d[0] > 0) {
        // Update successful
        const updatedDoctors = d[1]; // Array of updated doctor objects (if needed)
        return res.status(200).json({ message: "Your email is verified", updatedDoctors });
      } else {
        // No rows were updated
        return res.status(400).json({ message: "Invalid verification or email is already verified" });
      }
}
//---------------------------------------------------------------------
export const sendCode = async(req,res,next)=>{
    ...
    code= code();
    const user = await DoctorModel.update(
...
export const forgotPassword = async (req, res,next) => {
    ...
    //use email to retrieve the account.
    ...
    //add sendCode to the data base.type String default :null;
    if (doctor.sendCode != code) {
        //400:indicates that the request sent to the server is invalid or corrupted.
        return res.status(400).json({ message: 'Invalid code' });
    }
    //to encrypt the password.
    //parseInt : to return int.
    //creat env and put SALTROUND.
    //call bcrypt library.
    //import bcrypt from 'bcryptjs'
    doctor.password = await bcrypt.hash(password, parseInt(process.env.SALTROUND));
    //return the send code null in database
    doctor.sendCode = null;

    await doctor.save();
    //200:HTTP response status code 200 means success.
...
export const signin = async(req,res,next)=>{
    ...
    if(!doctor){
        return res.status(400).json({message:"data Invalid (id)"});
    }
    ...
    const token = jwt.sign({_id:doctor._id},process.env.LOGINSECRET,{expiresIn:'1h'});
    const refreshToken = jwt.sign({id:doctor._id},process.env.LOGINSECRET,{expiresIn:60*60*24*30});
